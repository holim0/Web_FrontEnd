...
1. @**types** : 타입을 정의하는 폴더
   - 여기다가 타입 다넣음
   - 타입을 한번이 아니라 여러번 사용하면 이곳에 넣기!!!
     ex) onclick 이벤트, onchange 이벤트는 event.ts 파일에 export type = onClick : (e: React~!~!~!`) => void

2. **components**
   - 함수형 컴포넌트파일들
   - layouts (헤더, 푸터 등)
   - common (재사용 컴포넌트)
   - Home (HomeForm.tsx <-- 프레젠터역할. HomeCard)

3. **pages** : nextjs 에서는 라우터
라우터에따른 페이지 (로직은 이곳에서)
4. **redux**
리덕스
